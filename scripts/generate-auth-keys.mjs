 ...
 *
 * Each JWK includes an RFC 7638 thumbprint `kid` so self-hosted customJwt
 * validation can match the kid header on signed tokens (JWT_KID).
 */
...
if (writeDir) {
  ...
  await writeFile(resolve(dir, "jwt_kid"), kid, "utf8")
  console.error(`Wrote JWT keys to ${dir}/jwt_private_key and ${dir}/jwks`)
  console.error(`JWT kid (thumbprint): ${kid}`)
}
