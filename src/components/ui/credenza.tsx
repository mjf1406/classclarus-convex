...
const Credenza = ({ children, ...props }: RootCredenzaProps) => {
  ...
  const CredenzaRoot = isDesktop ? Dialog : Drawer

  return (
    <CredenzaContext.Provider value={{ isDesktop }}>
      <CredenzaRoot {...props} {...(!isDesktop && { autoFocus: true })}>
        {children}
      </CredenzaRoot>
...
const CredenzaBody = ({ className, children, ...props }: CredenzaProps) => {
  return (
    <div className={cn('min-h-0 overflow-y-auto px-4 md:px-0', className)} {...props}>
      {children}
    </div>
  )
}
